...
class SettingsViewModel(private val prefs: AppPreferences) : ViewModel() {
    ...
    fun setSegmentMinutes(minutes: Int)   { viewModelScope.launch { prefs.setSegmentMinutes(minutes) } }
    ...
    fun setCbWmToken(token: String)       { viewModelScope.launch { prefs.setCbWmToken(token) } }
}
