...
object NotificationHelper {
    ...
    fun createChannel(context: Context) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            ).apply { description = "Notifications when a tracked streamer goes live or offline" }
    ...
    fun notifyStatusChange(
        ...
        val (title, body) = when {
            isLive(newStatus) && !isLive(oldStatus)  -> "🔴 $username is LIVE"     to "$site · Stream just started"
            !isLive(newStatus) && isLive(oldStatus)  -> "⚫ $username went offline" to "$site · ${newStatus.uppercase()}"
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            .setSmallIcon(R.mipmap.ic_launcher)   // ← app icon in notification panel
